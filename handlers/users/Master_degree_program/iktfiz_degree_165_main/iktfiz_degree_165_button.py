...
from keyboards.inline.degree_inline.info_degree_165_inline.deg_165_inbut import menu_165iktfiz_degree_button, \
    menu_165iktfiz_degree_button_1, back_deg_165_button
from keyboards.inline.degree_inline.main_degree_inbut import menu_iltfiz_degree_button_165
...
@dp.callback_query_handler(text="IANT_degree_4")
async def iant_1(c: CallbackQuery):
    ...
    await c.message.answer(f"Вы выбрали <b><ins>Институт авиации, наземного транспорта и энергетики</ins></b>\n"
                           f"Направления:", reply_markup=menu_165iktfiz_degree_button)


# 145
@dp.callback_query_handler(text="Shipbuilding_deg_4")
async def iant_deg_1(c: CallbackQuery):
    ...
    await c.message.answer(f"👩‍🏫☔️Практика и стажировки:\n"
                           ...
                           f"• АО «Зеленодольское проектно-конструкторское бюро», г. Зеленодольск, РТ\n"
                           f"• АО «Зеленодольский завод им. А.М. Горького», г. Зеленодольск, РТ",
                           reply_markup=back_deg_165_button)


# 165
@dp.callback_query_handler(text="Aircraft_construction_deg_4")
...
# Ирэт
@dp.callback_query_handler(text="IREF_CT_degree_4")
async def IANT_2(c: CallbackQuery):
    ...
    await c.message.answer("Вы выбрали <b><ins>Институт Радиоэлектроники, фотоники и цифровых технологий</ins></b>\n\n"
                           "Направления:", reply_markup=menu_165iktfiz_degree_button_1)


# 150
@dp.callback_query_handler(text="Transmission_degree_systems_iktfiz_4")
...
# 159
@dp.callback_query_handler(text="Design_technology_degree_4")
async def iret_iktfiz_1(c: CallbackQuery):
    await c.answer(cache_time=60)
...
# назад
@dp.callback_query_handler(text="back_165_deg_4")
async def b_177(c: CallbackQuery):
    ...
    await c.message.delete()
    await c.message.answer(f"Выбери факультет", reply_markup=menu_iltfiz_degree_button_165)


# исклик то назад к факультетам
@dp.callback_query_handler(text="back_deg_165")
async def b_177(c: CallbackQuery):
    await c.answer(cache_time=60)
